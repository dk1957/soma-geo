import { ch as getDefaultExportFromCjs, cP as normalizeUrl, cQ as isSameOrigin } from "./index-CSpjggkr.js";
import "node:events";
import "node:stream";
...
function analyzeHtml(html2, pageUrl, statusCode, responseTimeMs, redirectUrl = null) {
  const $2 = load(html2);
  const title = $2("title").first().text().trim();
  const metaDescription = $2('meta[name="description"]').first().attr("content")?.trim() ?? "";
  const canonical = $2('link[rel="canonical"]').first().attr("href") ?? null;
  const robotsMeta = $2('meta[name="robots"]').first().attr("content") ?? null;
  const ogTitle = $2('meta[property="og:title"]').first().attr("content") ?? null;
  const ogDescription = $2('meta[property="og:description"]').first().attr("content") ?? null;
  const ogImage = $2('meta[property="og:image"]').first().attr("content") ?? null;
  const h1s = [];
  $2("h1").each((_, el) => {
    h1s.push($2(el).text().trim());
  });
  const headingOrder = [];
  $2("h1, h2, h3, h4, h5, h6").each((_, el) => {
    const tag = "tagName" in el && typeof el.tagName === "string" ? el.tagName.toLowerCase() : null;
    if (tag) {
      const level = parseInt(tag.charAt(1), 10);
      if (!isNaN(level)) headingOrder.push(level);
    }
  });
  const bodyClone = $2("body").clone();
  ...
  $2("img").each((_, el) => {
    images.push({
      src: $2(el).attr("src") ?? null,
      alt: $2(el).attr("alt") ?? null
    });
  });
  ...
  $2("a[href]").each((_, el) => {
    ...
    const anchor = $2(el).text().replace(/\s+/g, " ").trim().slice(0, 200);
  ...
  $2('script[type="application/ld+json"]').each(() => {
    hasStructuredData = true;
  });
  const hreflangTags = [];
  $2('link[rel="alternate"][hreflang]').each((_, el) => {
