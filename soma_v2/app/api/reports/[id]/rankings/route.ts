 ...
 * GET /api/reports/[id]/rankings
 * 
 * Returns competitive rankings table data
 */
...
export async function GET(
  ...
  try {
    ...
    const rankings = await analyticsService.getIndustryRankings(
    ...
    const formattedRankings = rankings.map(r => ({
      brand: r.brand_name,
      isYourBrand: r.is_primary_brand,
      position: r.industry_rank || 0,
      positionChange: r.rank_change || 0,
      sentiment: Math.round(Number(r.avg_sentiment_score) * 100),
      sentimentChange: Number(r.sentiment_change?.toFixed(1)) || 0,
      visibility: Number(r.mention_rate?.toFixed(1)) || 0,
      visibilityChange: Number(r.mention_rate_change?.toFixed(1)) || 0,
      lvi: Number(r.lvi_score?.toFixed(1)) || 0
    }))
