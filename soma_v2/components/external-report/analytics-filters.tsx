...
import { Check, ChevronRight, Calendar as CalendarIcon, RotateCcw, Sparkles } from 'lucide-react'
...
interface ExternalAnalyticsFiltersProps {
...
export function ExternalAnalyticsFilters({ 
  filters, 
  onFiltersChange, 
  availableCompetitors,
  availableModels
}: ExternalAnalyticsFiltersProps) {
  const [datePopoverOpen, setDatePopoverOpen] = useState(false)
  const [calendarMonth, setCalendarMonth] = useState<Date>(
    filters.dateRange.from ? new Date(filters.dateRange.from) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
  )
  // Track whether we're picking the start or end of the range
  const [rangeSelectionStep, setRangeSelectionStep] = useState<'start' | 'end'>(
    filters.dateRange.from && filters.dateRange.to ? 'start' : filters.dateRange.from ? 'end' : 'start'
  )
  
  // Transform models for display
  const aiPlatformOptions = availableModels.map(model => ({
    value: model,
  ...
  const handleDatePreset = (days: number) => {
    ...
    setCalendarMonth(new Date(from))
  }
            ...
            <Popover open={datePopoverOpen} onOpenChange={setDatePopoverOpen}>
              ...
              <PopoverContent className="w-auto p-0" align="start" sideOffset={8}>
                <div className="p-5 pb-4">
                  ...
                  <div className="flex items-center gap-3 text-xs mb-4 p-3 bg-gray-50 rounded-lg">
                    <div className={cn(
                      "flex-1 p-2 rounded-md border-2 transition-colors",
                      rangeSelectionStep === 'start' ? "border-gray-900 bg-white" : "border-transparent"
                    )}>
                      <span className="block text-[10px] text-gray-400 uppercase tracking-wider font-medium mb-1">From</span>
                      <span className={cn(
                        "block text-sm font-medium",
                        filters.dateRange.from ? "text-gray-900" : "text-gray-400"
                      )}>
                        {filters.dateRange.from ? format(new Date(filters.dateRange.from), 'MMM d, yyyy') : 'Click a date'}
                      </span>
                    </div>
                    <div className="text-gray-300 text-lg">→</div>
                    <div className={cn(
                      "flex-1 p-2 rounded-md border-2 transition-colors",
                      rangeSelectionStep === 'end' ? "border-gray-900 bg-white" : "border-transparent"
                    )}>
                      <span className="block text-[10px] text-gray-400 uppercase tracking-wider font-medium mb-1">To</span>
                      <span className={cn(
                        "block text-sm font-medium",
                        filters.dateRange.to ? "text-gray-900" : "text-gray-400"
                      )}>
                        {filters.dateRange.to ? format(new Date(filters.dateRange.to), 'MMM d, yyyy') : 'Click a date'}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <Calendar
                    ...
                    onSelect={(range: DateRange | undefined) => {
                      if (rangeSelectionStep === 'start') {
                        // First click: set as new start date, clear end
                        updateFilters({
                          dateRange: {
                        ...
                        // Second click: set end date, complete range
                        updateFilters({
                          dateRange: {
                            from: range?.from ? range.from.toISOString().split('T')[0] : '',
                ...
                <div className="flex gap-2 p-5 pt-3 border-t border-gray-100">
                  ...
                  <Button 
                    size="sm" 
                    className="flex-1 bg-gray-900 hover:bg-gray-800 text-white rounded-lg text-xs h-9"
                    onClick={() => setDatePopoverOpen(false)}
