-- Complete Ground Truth Collections System
-- Created: 2025-09-08 12:00:00
-- Purpose: Create comprehensive ground truth data collection with proper brand/account relationships

-- Drop existing table if it exists (since user mentioned it was deleted)
DROP TABLE IF EXISTS ground_truth_collections CASCADE;

-- Create the complete ground_truth_collections table with all columns
CREATE TABLE ground_truth_collections (
    ...
    collection_type TEXT DEFAULT 'comprehensive' CHECK (collection_type IN ('quick', 'comprehensive', 'periodic')),
    competitor_data JSONB DEFAULT '{}',
    source_metadata JSONB DEFAULT '{}',
    processing_status TEXT DEFAULT 'pending' CHECK (processing_status IN ('pending', 'processing', 'completed', 'failed')),
    ldi_input_generated BOOLEAN DEFAULT FALSE,
    keywords_extracted TEXT[] DEFAULT '{}',
    competitors_discovered TEXT[] DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add indexes for better performance
CREATE INDEX idx_ground_truth_collections_user_id ON ground_truth_collections(user_id);
CREATE INDEX idx_ground_truth_collections_account_brand ON ground_truth_collections(account_id, brand_id);
CREATE INDEX idx_ground_truth_collections_processing_status ON ground_truth_collections(processing_status, created_at);
CREATE INDEX idx_ground_truth_collections_type_status ON ground_truth_collections(collection_type, processing_status);
CREATE INDEX idx_ground_truth_collections_brand_name ON ground_truth_collections(brand_name);
CREATE INDEX idx_ground_truth_collections_created_at ON ground_truth_collections(created_at DESC);
...
-- Enable RLS
ALTER TABLE ground_truth_collections ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can update own ground truth collections" ON ground_truth_collections
    FOR UPDATE USING (
        auth.uid() = user_id 
        OR 
        account_id IN (
...
CREATE POLICY "Users can delete own ground truth collections" ON ground_truth_collections
    FOR DELETE USING (
        auth.uid() = user_id 
        OR 
        account_id IN (
...
CREATE TRIGGER update_ground_truth_collections_updated_at
    BEFORE UPDATE ON ground_truth_collections
    FOR EACH ROW
...
GRANT ALL ON ground_truth_collections TO authenticated;

-- Add comments for documentation
COMMENT ON TABLE ground_truth_collections IS 'Complete ground truth research data with proper brand/account relationships for LDI calculation';
