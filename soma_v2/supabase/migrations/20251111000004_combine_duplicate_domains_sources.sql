-- Update source citation analysis to combine duplicate domains and collect all unique URLs
-- This prevents duplicate domains in the Sources & Citations section
...
BEGIN
  ...
  domain_stats AS (
    -- Aggregate by domain only (not by type)
    SELECT 
      cd.domain,
      -- Use the most common type for this domain
      ...
      -- Collect all unique URLs for this domain
...
-- Comment
COMMENT ON FUNCTION get_source_citation_analysis IS 'Analyzes source citations from response_analysis.sources_cited JSONB, aggregating by domain (combining duplicates) and collecting all unique URLs for each domain';
