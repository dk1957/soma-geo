-- Fix recent brand mentions function to return proper structure for frontend
-- Returns all responses (not just where brand is mentioned) to show AI conversation activity
-- Fixes: Date (relative time handled in frontend), Position (null when not mentioned), 
--        Mentions (shows competitors), Sources (from sources_cited)
...
COMMENT ON FUNCTION get_recent_brand_mentions IS 'Returns recent AI responses with prompt, response snippet, brand position (null if not mentioned), all mentioned brands with logos, model info, sources, and date. Shows all responses for the brand, not just ones where brand was mentioned.';
