-- Migration: Rebalance LVI weights to prioritize visibility and position
-- Date: 2026-04-11
-- Description: Updates LVI formula weights from {Vis:0.30, Pos:0.25, Cit:0.25, Sent:0.20}
--              to {Vis:0.35, Pos:0.30, Cit:0.15, Sent:0.20}.
--
--   In GEO (Generative Engine Optimization), being mentioned (visibility) and mentioned
--   early (position) are the primary goals. Citation rate disproportionately inflated
--   scores for established brands with well-known domains (100% citation rate) while
--   penalizing newer brands that AI models mention but can't cite with URLs.
--
-- UPDATED FORMULA:
--   LVI = Visibility*0.35 + Position*0.30 + Citation*0.15 + Sentiment*0.20

-- ============================================================================
-- Update get_brand_stats — new canonical weights
-- ============================================================================
...
-- ============================================================================
-- Update get_lvi_timeseries — new canonical weights
-- ============================================================================
...
-- ============================================================================
-- Update get_industry_rankings — new canonical weights
-- ============================================================================
...
BEGIN
  ...
  previous_metrics AS (
    SELECT 
      ...
      -- Same LVI formula for previous period
      LEAST(100,
        LEAST(35, (COUNT(CASE WHEN ra.brand_mentioned THEN 1 END)::NUMERIC / 
         NULLIF(COUNT(*), 0)::NUMERIC) * 100 * 0.35) +
        (CASE 
...
-- ============================================================================
-- Recalculate stored LVI scores in daily_brand_metrics and daily_competitor_metrics
-- using new weights: Vis*0.35 + Pos*0.30 + Cit*0.15 + Sent*0.20
-- ============================================================================

-- Update daily_brand_metrics
UPDATE daily_brand_metrics SET
  lvi_score = CASE 
    WHEN visibility_rate = 0 THEN 0
    ELSE LEAST(100, GREATEST(0, ROUND(
      (visibility_rate * 0.35) +
      (CASE
        WHEN avg_brand_rank IS NULL OR avg_brand_rank = 0 THEN 0
        ELSE GREATEST(0, (1.0 - (avg_brand_rank - 1.0) / 9.0)) * 100
      END * 0.30) +
      (citation_rate * 0.15) +
      (((COALESCE(avg_sentiment, 0) + 1) / 2) * 100 * 0.20),
      2
    )))
  END;

-- Update daily_competitor_metrics
UPDATE daily_competitor_metrics SET
  lvi_score = CASE 
    WHEN visibility_rate = 0 THEN 0
    ELSE LEAST(100, GREATEST(0, ROUND(
      (visibility_rate * 0.35) +
